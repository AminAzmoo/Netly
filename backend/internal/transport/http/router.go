...
func SetupRoutes(app *fiber.App, cfg RouterConfig) ports.InstallerService {
	...
	installerService := services.NewInstallerService(timelineRepo, nodeRepo, cfg.Logger, cfg.EnableTaskCorrelation, cfg.Config.Security.PublicURL)
	...
	agentHandler := handlers.NewAgentHandler(nodeService, taskService, cfg.Logger, keyManager)
	...
	networkHandler := handlers.NewNetworkHandler(handlers.NetworkHandlerConfig{
		...
		IPAMConfig: handlers.IPAMConfigInfo{
			...
			IPv6CIDR: ipamConfig.IPv6CIDR,
		},
		PortAMConfig: handlers.PortAMConfigInfo{
			MinPort: portamConfig.MinPort,
			MaxPort: portamConfig.MaxPort,
		},
	})
	network := api.Group("/network", httpmw.AdminAuth(cfg.Config))
	network.Get("/stats", networkHandler.GetNetworkStats)
